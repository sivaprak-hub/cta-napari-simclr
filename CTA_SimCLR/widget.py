...
class CalciumControls(QWidget):
    """Left-panel controls — napari plugin widget (SimCLR-enhanced fork)."""
    ...
    def __init__(self, napari_viewer: 'napari.viewer.Viewer'):
        ...
        # SimCLR epochs spinner — always visible since all three methods always run
        self.spin_epochs = QSpinBox()
        self.spin_epochs.setRange(20, 500)
        self.spin_epochs.setValue(100)
        self.spin_epochs.setSingleStep(10)
        self.spin_epochs.setToolTip(
            "SimCLR training epochs per recording.\n"
            "20–50 for quick exploration, 100 (default) for good embeddings,\n"
            "200+ for maximum quality on clean recordings."
        )
        ...
        l_param.addRow("SimCLR Epochs:", self.spin_epochs)
        ...
        self.lbl_ncc   = QLabel("NCC cluster score: —")
        self.lbl_ncc.setStyleSheet("color: #FFB300;")
        self.lbl_emb   = QLabel("Embedding score: —")
        self.lbl_emb.setStyleSheet("color: #FF7043;")
        l_act.addWidget(self.btn_run)
        l_act.addWidget(self.prog)
        l_act.addWidget(self.lbl_beats)
        l_act.addWidget(self.lbl_sync)
        l_act.addWidget(self.lbl_ncc)
        l_act.addWidget(self.lbl_emb)
    ...
    def start_analysis(self):
        ...
        self._last_params = {
            ...
            'use_fps':         is_fps,
            'val':             self.spin_val.value(),
            'n_simclr_epochs': self.spin_epochs.value(),
        }
    ...
    def on_analysis_done(self, results):
        ...
        ncc_score = results.get('ncc_score', None)
        emb_score = results.get('emb_score', None)
        self.lbl_ncc.setText(f"NCC cluster score: {ncc_score:.3f}" if ncc_score is not None else "NCC cluster score: —")
        self.lbl_emb.setText(f"Embedding score: {emb_score:.3f}"   if emb_score is not None else "Embedding score: —")

        scale = (self.spin_bin.value(), self.spin_bin.value())

        for name in ['Clusters (HDBSCAN)', 'Clusters (NCC)', 'Clusters (SimCLR)',
                     'Clusters', 'Pulsatility', 'Wave Map', 'Selection']:
        ...
        # All three cluster maps as separate layers
